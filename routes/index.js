...
router.get("/test2", function (req, res) {
  ...
  res.setHeader(
    ...
    "https://pinmi-node-server.herokuapp.com/test2"
  );
  res.send({
...
router.get("/test4", function (req, res) {
  ...
  res.setHeader(
    ...
    "Access-Control-Allow-Origin, Access-Control-Allow-Methods, Origin, X-Requested-With, Content-type, Accept, Vary"
  );
  res.setHeader("Access-Control-Allow-Origin", "*");
...
router.get("/s3/:archiveId", function (req, res) {
  ...
  var s3URL =
    "https://pin-mi-project.s3.amazonaws.com/" +
    apiKey +
    "/" +
    archiveId +
    "/archive.mp4";
  ...
  opentok.getArchive(archiveId, function (err, archive) {
    if (err) {
      ...
      res.status(500).send({ error: "getArchive error:" + err });
      return;
    } else if (archive.status == "uploaded") {
      //if vonage has archive data set yet
      res.send({
        arcStatus: "uploaded",
      ...
      res
        .status(202)
        .send({ arcStatus: "stopped", message: "process not complete." });
    } else if (archive.status == "available") {
      res.send({
        arcStatus: "available",
        message: "Archive not saved to s3. Saved in Vonage. Follow redirect",
        redirect: "/archive/:archiveId",
      });
